...
Item {
    ...
    required property SpeechViewModel speechViewModel
    ...
    ScrollView {
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    ToolButton {
                        id: speakButton
                        visible: root.speechViewModel.available
                        Layout.preferredWidth: 40
                        Layout.preferredHeight: 40
                        focusPolicy: Qt.NoFocus
                        ToolTip.visible: speakButton.hovered
                        ToolTip.text: root.speechViewModel.speaking
                                      ? qsTr("Dừng")
                                      : qsTr("Nghe phát âm")
                        background: Rectangle {
                            radius: 4
                            color: speakButton.hovered ? "#2c2c34" : "transparent"
                            Behavior on color { ColorAnimation { duration: 120 } }
                        }
                        contentItem: Label {
                            text: root.speechViewModel.speaking ? "⏹" : "🔊"
                            font.pixelSize: 20
                            color: root.speechViewModel.speaking ? "#f0c040" : "#a0a0a8"
                            horizontalAlignment: Text.AlignHCenter
                            verticalAlignment: Text.AlignVCenter
                        }
                        onClicked: {
                            if (root.speechViewModel.speaking)
                                root.speechViewModel.stop()
                            else
                                root.speechViewModel.speakEntry(root.readingHira, root.word)
                        }
                    }
                    Label {
                        visible: !root.speechViewModel.available
                        text: qsTr("🔇 chưa có giọng tiếng Nhật")
                        font.pixelSize: 11
                        color: "#5c5c64"
                        padding: 4
                        background: Rectangle { color: "#22222a"; radius: 3 }
                    }
                    Button {
